...
module.exports.run = async (args, api, db, event, input, msgID, senderID, threadID, src, prefix) => {
  let query = args.join(" ");
  if (query.length === 0) {
    api.sendMessage(`⚠️Invalid use of command!\n💡${prefix}playmp4 song_title`, threadID, msgID);
    return;
  }
  ...
  try {
    ...
    const file = fs.createWriteStream(`${src}music.mp4`);
    ...
    async function main() {
      ...
      api.sendMessage({
        body: messageBody,
        attachment: fs.createReadStream(`${src}music.mp4`)
      }, threadID, msgID).then(resp => {deleteAttach();})
    }
    
    async function deleteAttach() {
      const filePath = src + 'music.mp4';
